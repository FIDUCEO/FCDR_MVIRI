...
def l15_to_L10_name(rect2lpname,s_datestring,sat="MET7"):
  """
  ...
  """
  prepend="/"#"/tcenas/proj/eraclim/data/imag2tg/"#"/tcenas/proj/eraclim/data/imag2tg" #necessary while converted imag2tg are not yet available on NAS
  ...
  outfolder.replace("//","/")
  wildcard=outfolder
  imag2tgname=glob.glob(wildcard)
  #print imag2tgname[0]
  try:
    return imag2tgname[0]
  except IndexError:
    print "WARNING: no fitting L1.0 file found using wildcard: "
    print wildcard
    wildcard=outfolder.replace("Repro","Repro_Temp")
    imag2tgname=glob.glob(wildcard)
    try:
      return imag2tgname[0]
    except IndexError:
      print "WARNING: no fitting L1.0 file found using wildcard: "
      print wildcard
      wildcard=wildcard.replace(sat,sat+"/"+sat)
      #wildcard="/tcenas/proj/eraclim/data/imag2tg"+outfolder
      imag2tgname=glob.glob(wildcard)
      try:
        return imag2tgname[0]
      except IndexError:
...
#READ TELEMETRY OFFSETS
  teletab=np.genfromtxt("../config/Telemetry_offsets.csv",delimiter=",",dtype="str",skip_header=1)
...
def header(f,start=1,end=39):
  #open file
	...
	g=g+dims[i]
      ...
      #debug
      #if rq==2:
	#print getattr(struc, 'dsDist_from_earth_centre')
      #if rq==3:
	#print getattr(struc, 'flSpaceCornersVISS')
